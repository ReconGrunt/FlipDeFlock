...
static void atk_build(ReconApp* app) {
    ...
    for(size_t i = 0; i < app->deauth_count; i++) {
        ...
        atk_append(
            text, sizeof(text), &pos, " %lu frames  %lu:%02lu\n", (unsigned long)d->count,
    ...
    if(app->esp_attack_tick) {
        ...
        if(st != AttackStatusEnded) {
            ...
            atk_append(
                text, sizeof(text), &pos, " rate %lu  %lu:%02lu\n",
                (unsigned long)app->esp_attack_value, (unsigned long)(secs / 60),
