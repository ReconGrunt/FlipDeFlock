...
def read_flash_args(build_dir: Path):
    ...
    for line in fa.read_text().splitlines():
        ...
        path = build_dir / name
        if not path.exists():
            sys.exit(f"ERROR: flash_args lists {name} at {off_s}, but that file "
...
def verify(out: Path, chunks, chip: str):
    ...
    if expected is not None and boot_off != expected:
        sys.exit(f"ERROR: flash_args puts the bootloader at {boot_off:#x}, but "
    ...
    if len(data) <= boot_off:
        sys.exit(f"ERROR: merged image is {len(data)} bytes, shorter than the "
                 f"bootloader offset {boot_off:#x}.")
    if data[boot_off] != ESP_IMAGE_MAGIC:
        sys.exit(f"ERROR: expected ESP image magic {ESP_IMAGE_MAGIC:#04x} at "
    ...
    for off, path in chunks[1:]:
        if len(data) <= off:
            sys.exit(f"ERROR: {path.name} should start at {off:#x} but the image "
    ...
    if len(data) > useful * 2:
        sys.exit(f"ERROR: merged image is {len(data)} bytes for {useful} bytes "
