...
void suite_attack_triage(void) {
    ...
    CHECK_INT_EQ(
        attack_triage_status(2, now - 500, now - 100, now, 0, 0, 0), AttackStatusBlip);
    // High count but no duration: a single dense instant is still a blip until it
    // persists (span below sustained).
    CHECK_INT_EQ(
        attack_triage_status(200, now - 1000, now - 100, now, 0, 0, 0), AttackStatusBlip);
    // Long span but almost no frames: something ticked twice over 10 s -- churn,
    // not a flood.
    CHECK_INT_EQ(
        attack_triage_status(3, now - 10000, now - 100, now, 0, 0, 0), AttackStatusBlip);
    ...
    CHECK_INT_EQ(
        attack_triage_status(500, now - 100, now - 5000, now, 0, 0, 0), AttackStatusBlip);
